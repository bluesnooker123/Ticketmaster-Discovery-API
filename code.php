...
*/
	...
	curl_setopt_array($curl1, array(
	  CURLOPT_URL => 'https://app.ticketmaster.com/discovery/v2/events.json?keyword=rockies&source=ticketmaster&locale=en-us,en-ca&size=10&apikey=HNaHnprAE1WbPN3WwnIQ09bYjsDTQbpS&page=0',
	...
	for($i = 0; $i< $jsonArr1['page']['totalPages'];$i++){
		$curl = curl_init();

		curl_setopt_array($curl, array(
		  CURLOPT_URL => 'https://app.ticketmaster.com/discovery/v2/events.json?keyword=rockies&source=ticketmaster&locale=en-us,en-ca&size=10&apikey=HNaHnprAE1WbPN3WwnIQ09bYjsDTQbpS&page='.$i,
		  CURLOPT_RETURNTRANSFER => true,
		  CURLOPT_ENCODING => '',
		  CURLOPT_MAXREDIRS => 10,
		  CURLOPT_TIMEOUT => 0,
		...
		foreach($jsonArr['_embedded']['events'] as $key => $val){
			...
			if(mysqli_num_rows($checkEvent)>0){
					...
					if(isset($salesArr['presales'])){
						...
						if(count($salesPresalesArr)>0){
							foreach($salesPresalesArr as $presalesKey=>$presalesValue){
								...
								$names = (isset($presalesValue['name']))? $presalesValue['name']:'';
								echo "<br>NAME:".$names."<br>";
								$insertPresales = mysqli_query($conn,"insert into events_sales_presales (events_id, startDateTime, endDateTime, name)values('$eventID','".$presalesValue['startDateTime']."','".$presalesValue['endDateTime']."','".($names)."')");
							}
						}
					}
				/*add salses end*/
				...
				if(count($eventDates)>0){
					...
					$status = $eventDates['status'];
					$timezone = (isset($eventDates['timezone']))?$eventDates['timezone']:'';
					$lTime = (isset($start['localTime']))? $start['localTime'] : "";
					$dTime = (isset($start['dateTime']))? $start['dateTime'] : "";
					$insertDates = mysqli_query($conn,"insert into events_dates (events_id, `localDate`, `localTime`, `dateTime`, dateTBD, dateTBA, timeTBA, noSpecificTime, timezone, code, spanMultipleDays)values($eventID,'".$start['localDate']."','".$lTime."','".$dTime."','".$start['dateTBD']."','".$start['dateTBA']."','".$start['timeTBA']."','".$start['noSpecificTime']."','".$timezone."','".$status['code']."','".$eventDates['spanMultipleDays']."');"); 
				}
